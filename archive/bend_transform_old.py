...
class FoldDefinition:
    ...
    - The finite extent along the fold axis (where the fold actually reaches)
    """
    ...
    # Finite extent along the fold axis (relative to position)
    # None means infinite extent (affects entire axis)
    extent_min: Optional[float] = None  # Min projection along axis
    extent_max: Optional[float] = None  # Max projection along axis
    ...
    def point_in_extent(self, point: tuple[float, float]) -> bool:
        """Check if a point is within the fold's finite extent along the axis."""
        if self.extent_min is None or self.extent_max is None:
            return True  # Infinite extent

        # Project point onto fold axis
        dx = point[0] - self.position[0]
        dy = point[1] - self.position[1]
        along_axis = dx * self.axis[0] + dy * self.axis[1]

        # Check if within extent (with small tolerance)
        tolerance = 0.1
        return self.extent_min - tolerance <= along_axis <= self.extent_max + tolerance

    @classmethod
    def from_marker(cls, marker: FoldMarker) -> 'FoldDefinition':
        """Create FoldDefinition from a FoldMarker."""
        # Compute finite extent from marker line endpoints
        center = marker.center
        axis = marker.axis

        # Project all marker line endpoints onto the axis
        def project_along_axis(p: tuple[float, float]) -> float:
        ...
        projections = [project_along_axis(p) for p in marker_points]
...
def classify_point(
    ...
    A point is only affected by a fold if it is within the fold's finite extent
    along the fold axis. Points outside this extent are classified as BEFORE
    ...
    """
    # First check if point is within the fold's finite extent along the axis
    # If not, the fold doesn't affect this point at all
    if not fold.point_in_extent(point):
        return (PointClassification.BEFORE, 0.0)
...
def transform_point(
    ...
    Args:
        ...
        preserve_order: If True, use folds in given order (for region-based recipes).
                       If False, sort by perpendicular distance (for parallel folds).
    ...
    # Skip sorting if preserve_order is True (for region-based transformation)
    if len(folds) > 1 and not preserve_order:
    ...
    for i, fold in enumerate(folds):
        ...
        if i == 0:
            ...
            axis_p1_3d = transform_point(axis_p1, folds[:i], preserve_order)
            axis_p2_3d = transform_point(axis_p2, folds[:i], preserve_order)
            ...
            pivot_3d = transform_point(pivot_2d, folds[:i], preserve_order)

            # Perpendicular direction in 3D
            perp_point_2d = (pivot_2d[0] + perp[0], pivot_2d[1] + perp[1])
            perp_point_3d = transform_point(perp_point_2d, folds[:i], preserve_order)
            perp_3d = _normalize3((
...
def transform_point_with_recipe(
    point: tuple[float, float],
    fold_recipe: list[tuple[FoldDefinition, str]]
) -> tuple[float, float, float]:
    """
    Transform a 2D point using a fold recipe with pre-determined classifications.

    Unlike transform_point() which classifies points geometrically, this function
    uses the classification from the fold_recipe directly. This ensures that
    regions remain flat (planar) - all points in a region get the same treatment.

    Args:
        point: 2D point (x, y)
        fold_recipe: List of (FoldDefinition, classification) tuples where
                    classification is "IN_ZONE" or "AFTER"

    Returns:
        3D point (x, y, z) after transformation
    """
    if not fold_recipe:
        return (point[0], point[1], 0.0)
    ...
    for i, (fold, recipe_classification) in enumerate(fold_recipe):
        # Convert string classification to PointClassification
        if recipe_classification == "IN_ZONE":
            classification = PointClassification.IN_ZONE
        elif recipe_classification == "AFTER":
            classification = PointClassification.AFTER
        else:
            # Unknown classification - skip this fold
            continue

        # Get geometric distance for positioning (but use recipe classification!)
        _, perp_dist = _project_onto_axis(point, fold.position, fold.axis)
        half_width = fold.zone_width / 2

        # Compute dist based on classification
        if classification == PointClassification.IN_ZONE:
            # Distance from start of zone (0 to zone_width)
            dist = perp_dist + half_width
            # Clamp to zone boundaries for safety
            dist = max(0, min(dist, fold.zone_width))
        else:  # AFTER
            # Distance beyond zone end
            dist = perp_dist - half_width
            # For AFTER, dist should be positive (beyond zone)
            dist = max(0, dist)
        ...
        if i == 0:
            ...
            # Transform axis and pivot through earlier folds
            earlier_recipe = fold_recipe[:i]
            axis_p1 = fold.position
            axis_p2 = (fold.position[0] + fold.axis[0], fold.position[1] + fold.axis[1])
            axis_p1_3d = transform_point_with_recipe(axis_p1, earlier_recipe)
            axis_p2_3d = transform_point_with_recipe(axis_p2, earlier_recipe)
            fold_axis_3d = _normalize3((
                axis_p2_3d[0] - axis_p1_3d[0],
                axis_p2_3d[1] - axis_p1_3d[1],
                axis_p2_3d[2] - axis_p1_3d[2]
            ...
            pivot_3d = transform_point_with_recipe(pivot_2d, earlier_recipe)

            # Perpendicular direction in 3D
            perp_point_2d = (pivot_2d[0] + perp[0], pivot_2d[1] + perp[1])
            perp_point_3d = transform_point_with_recipe(perp_point_2d, earlier_recipe)
            perp_3d = _normalize3((
                perp_point_3d[0] - pivot_3d[0],
                perp_point_3d[1] - pivot_3d[1],
                perp_point_3d[2] - pivot_3d[2]
        ...
        if classification == PointClassification.IN_ZONE:
            # Point is on the cylindrical arc
            arc_fraction = dist / fold.zone_width if fold.zone_width > 0 else 0
            theta = arc_fraction * fold.angle
            ...
            # Compute arc end position (full angle)
            if abs(fold.angle) < 1e-6:
                arc_perp = fold.zone_width
                arc_up = 0.0
            else:
...
def compute_surface_normal_with_recipe(
    point: tuple[float, float],
    fold_recipe: list[tuple[FoldDefinition, str]]
) -> tuple[float, float, float]:
    """
    Compute surface normal using a fold recipe with pre-determined classifications.
    ...
    for fold, recipe_classification in fold_recipe:
        # Convert string classification
        if recipe_classification == "IN_ZONE":
            classification = PointClassification.IN_ZONE
        elif recipe_classification == "AFTER":
            classification = PointClassification.AFTER
        ...
        # Get perpendicular distance for arc position calculation
        _, perp_dist = _project_onto_axis(point, fold.position, fold.axis)
        half_width = fold.zone_width / 2

        if classification == PointClassification.IN_ZONE:
            # Compute arc position within zone
            dist = perp_dist + half_width
            dist = max(0, min(dist, fold.zone_width))
            arc_fraction = dist / fold.zone_width if fold.zone_width > 0 else 0
            theta = arc_fraction * fold.angle
...
def transform_point_with_regions(
    point: tuple[float, float],
    regions: list,  # list[Region] - avoid circular import
    all_fold_markers: list[FoldMarker]
) -> tuple[float, float, float]:
    """
    Transform a 2D point using region-based fold recipes.

    This function handles non-parallel folds on different branches of the board
    by using each region's fold_recipe to determine which folds affect points
    in that region. The classification from the recipe is used directly,
    ensuring regions remain planar.

    Args:
        point: 2D point (x, y)
        regions: List of Region objects with fold_recipe computed
        all_fold_markers: List of all FoldMarker objects (for fallback)
    ...
    if region is None:
        # Point is outside all regions - use fallback with all folds
        folds = create_fold_definitions(all_fold_markers)
        return transform_point(point, folds)
    ...
    for fold_marker, classification in region.fold_recipe:
        ...
        fold_recipe.append((fold_def, classification))

    # Transform using recipe-based function (uses pre-determined classifications)
    return transform_point_with_recipe(point, fold_recipe)


def compute_surface_normal_with_regions(
    point: tuple[float, float],
    regions: list,  # list[Region]
    all_fold_markers: list[FoldMarker]
) -> tuple[float, float, float]:
    """
    Compute surface normal using region-based fold recipes.

    The classification from the recipe is used directly, ensuring regions
    have consistent normals (planar regions stay planar).
    ...
    if region is None:
        folds = create_fold_definitions(all_fold_markers)
        return compute_surface_normal(point, folds)
    ...
    # Use recipe-based function (uses pre-determined classifications)
    return compute_surface_normal_with_recipe(point, fold_recipe)
